            ...
            if (isInt(req.params.id)) {
                ...
                if (userData) {
                    ...
                    if (userInitiator["is_admin"] || userData["id"] == userInitiator["id"]) {
                        ...
                        res.status(200).json({success: true, msg: await User.find({ id: req.params.id }, 'id register_date first_name last_name email password dob avatar badge_id points xp is_admin is_locked play_history comments title_ratings quiz_ratings seen favorites prizes_reedemed stats')});
                    } else {
